...
def calc_triangle(ex: str, tri: list, amount: float) -> Optional[dict]:
    ...
    for route_nodes in ([A, B, C, A], [A, C, B, A]):
        ...
        opp = {
            ...
            "timestamp":   time.time(),
            "id": hashlib.md5(f"{ex}{route}{round(profit_pct,2)}".encode()).hexdigest()[:8],
        }
            ...
            if G.running:
                ...
                for opp in opps[:3]:
                    ...
                    if oid not in G.alert_cache:
                        ...
                        threading.Timer(300, G.alert_cache.discard, [oid]).start()
...
def main_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("▶️ Iniciar" if not G.running else "✅ Activo", callback_data="cb_start"),
...
def status_text() -> str:
    ...
    m = rem // 60
    last = f"{int(time.time() - G.stats['last_scan'])}s atrás" if G.stats["last_scan"] else "—"
    return (
        f"🤖 *Arbitrage Bot*\n"
        f"━━━━━━━━━━━━━━━━━━\n"
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(status_text(), parse_mode="Markdown", reply_markup=main_kb())
...
async def cmd_help(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/balances — Saldos\n"
        "/status — Estado\n"
        "/setamount `<valor>` — Volume mínimo 10\n"
...
async def cmd_scan(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if opps:
        ...
        for i, o in enumerate(opps[:5], 1):
            lines.append(f"*{i}.* `{o['exchange'].upper()}` — `{o['route']}`\n"
                         f"   +{o['profit_pct']:.3f}% (+${o['profit_usdt']:.4f})\n")
        await msg.edit_text("\n".join(lines), parse_mode="Markdown")
    else:
        await msg.edit_text("❌ Sem oportunidades agora.")
...
async def cmd_status(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        status_text() + f"\n🌐 `{RENDER_URL}`", parse_mode="Markdown"
    )
...
async def cmd_set_amount(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if val < 10:
            await update.message.reply_text("❌ Mínimo: 10 USDT"); return
        G.amount = val
        await update.message.reply_text(f"✅ Volume: *${val} USDT*", parse_mode="Markdown")
    except (IndexError, ValueError):
        await update.message.reply_text("Uso: `/setamount 10`", parse_mode="Markdown")
...
async def cmd_set_profit(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        val = float(ctx.args[0])
        if not 0.05 <= val <= 20:
            await update.message.reply_text("❌ Entre 0.05% e 20%"); return
        G.min_profit = val
        await update.message.reply_text(f"✅ Profit mínimo: *{val}%*", parse_mode="Markdown")
    except (IndexError, ValueError):
        await update.message.reply_text("Uso: `/setprofit 0.3`", parse_mode="Markdown")
...
async def cmd_autotrade(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Auto-Trade: {label}{warn}", parse_mode="Markdown")
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "cb_start":
        G.running = True
        await q.edit_message_text("✅ *Bot INICIADO!*", parse_mode="Markdown", reply_markup=main_kb())
    elif d == "cb_stop":
        G.running = False
        await q.edit_message_text("⏹️ *Bot PARADO.*", parse_mode="Markdown", reply_markup=main_kb())
    elif d == "cb_scan":
        ...
        await q.edit_message_text(f"✅ Volume: `${G.amount} USDT`",
                                  parse_mode="Markdown", reply_markup=main_kb())
    elif d.startswith("cb_pft_"):
        m = {"02": 0.2, "03": 0.3, "05": 0.5, "10": 1.0}
        G.min_profit = m.get(d.replace("cb_pft_", ""), 0.3)
        await q.edit_message_text(f"✅ Profit: `{G.min_profit}%`",
                                  parse_mode="Markdown", reply_markup=main_kb())
    elif d == "cb_toggle_auto":
        G.auto_trade = not G.auto_trade
        label = "ON ✅" if G.auto_trade else "OFF ❌"
        warn  = "\n\n⚠️ Usa dinheiro REAL!" if G.auto_trade else ""
        await q.edit_message_text(f"Auto-Trade: *{label}*{warn}",
                                  parse_mode="Markdown", reply_markup=main_kb())
    elif d == "cb_dashboard":
        ...
        await q.edit_message_text(status_text(), parse_mode="Markdown", reply_markup=main_kb())
...
header p{font-size:.75em;color:var(--muted)}
...
.kc{background:var(--card);border:1px solid var(--border);border-radius:10px;padding:14px}
...
.pf{height:100%;background:var(--accent);animation:pr 10s linear infinite}
...
<main>
  ...
  <div class="two">
    <div class="sec">
      ...
      <div class="eg" id="exg"><div class="ec"><div class="en">A carregar...</div></div></div>
    </div>
    <div class="sec">
...
<footer>Actualiza a cada 10s<div class="prog"><div class="pf"></div></div></footer>
<script>
...
async function load(){
  try{const r=await fetch('/api');const d=await r.json();render(d)}catch(e){console.warn(e)}
}
function render(d){
  const s=d.stats||{},ex=d.exchanges||{},op=d.opportunities||[],tr=d.trades||[],cf=d.config||{};
  const run=s.running;
  ...
  if(s.start_time)document.getElementById('kup').textContent=upt(Math.round(Date.now()/1e3-s.start_time));
  ...
  if(Object.keys(ex).length){
    eg.innerHTML=Object.entries(ex).map(([n,i])=>`
      <div class="ec">
        ...
        ${i.error?`<div style="font-size:.65em;color:var(--red);margin-top:3px">${i.error.slice(0,50)}</div>`:''}
      </div>`).join('');
  }
  ...
  if(op.length){
    od.innerHTML=op.slice(0,10).map(o=>{
      ...
      return `<div class="opp ${hot?'hot':''}">
        ...
        <div class="sp">${st}</div>
        <div style="font-size:.68em;color:var(--muted);margin-top:5px">Vol: $${f2(o.amount)} · ID: ${o.id}</div>
      </div>`;
    }).join('');
  }else{od.innerHTML='<div class="empty">🔍 Sem oportunidades lucrativas neste momento</div>'}
  ...
  if(tr.length){
    ...
    hw.innerHTML=`<table class="tbl"><thead><tr>
      <th>Hora</th><th>Exchange</th><th>Rota</th><th>Lucro $</th><th>%</th><th>Estado</th>
    </tr></thead><tbody>${rows}</tbody></table>`;
  }else{hw.innerHTML='<div class="empty">Sem trades ainda</div>'}
}
...
def main():
    ...
    log.info("🌐 Flask em porta %d", PORT)
